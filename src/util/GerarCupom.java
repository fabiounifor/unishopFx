...
public class GerarCupom {
    ...
    public String geraCupomSangriaCaixa(int codigoCaixa, int codigoUsuario, Double ValorSangriaAtual) throws IOException {
        String textoParaImprimir = "";
        try {
            modelUsuario = controllerUsuario.getUsuarioController(codigoUsuario);
            modelCaixa = controllerCaixa.retorarCaixaControler(codigoCaixa);
            modelEmpresaCidadeEstado = controllerEmpresaCidadeEstado.getEmpresaCidadeEstadoController(1);
            ...
            textoParaImprimir = "======="+(modelEmpresaCidadeEstado.getModelEmpresa().getNomeFantasia() + "=====" +"\n\r"
                    ...
                    + modelEmpresaCidadeEstado.getModelCidade().getNome() + "-" + modelEmpresaCidadeEstado.getModelEstado().getUf() + "\n\r"
                    + modelEmpresaCidadeEstado.getModelEmpresa().getCnpj() + "\n\r"
                    + "Sangria do caixa Número: " + modelCaixa.getCaixaNumero() + "\n\r" 
                    + "Impressao:" + data1 + "-" + hora1 + "\n\r"
                    + "------------------------------------\n\r"
                    + "          VALOR SANGRIA        \n\r"
                    + "------------------------------------\n\r"
                    + "EFETUADA POR "+modelUsuario.getNome()+"\n\r"
                    + "TOTAL DE SANGRIA  =           " + (bLMascaras.arredondamentoComPontoDuasCasasDouble(ValorSangriaAtual))  + "\n\r"
                    + "VALOR DA SANGRIA DESSE CAIXA= " + (modelCaixa.getSangria())  + "\n\r"
                    + "--------------------------------\n\r"
                    + "--------------------------------\n\r"
                    + "--------------------------------\n\r"
                    + "\n\r\n\r\n\r\n\r") ;
        ...
        try {
            ...
            String conteudoFor = "<html><head></head><body>" + "---   "+ modelCaixa.getCaixaNumero() + "-----   " + modelCaixa.getCartao() + "-------   " + modelCaixa.getDinheiro() + "<br>" ;
    ...
    public String geraCupomCaixaEmail(int numeroCaixa) throws IOException {
        ...
        try {
            ...
            String conteudoFor = "<html>" + "---   "+ modelCaixa.getCaixaNumero() + "-----   " + modelCaixa.getCartao() + "-------   " + modelCaixa.getDinheiro() + "<br>\r";
            String usuario = modelSessaoUsuario.nome;
            //armazenar em uma string o texto(cupom) para imprimir
            textoParaEnviar = (modelEmpresaCidadeEstado.getModelEmpresa().getNomeFantasia() + "<br>\r" + modelEmpresaCidadeEstado.getModelEmpresa().getEndereco() + "<br>\r"                    + modelEmpresaCidadeEstado.getModelCidade().getNome() + "-" + modelEmpresaCidadeEstado.getModelEstado().getUf() + "<br>" + "<p>"
                    + modelEmpresaCidadeEstado.getModelEmpresa().getCnpj() + "<br> \r" 
                    + "OPERADOR: " + usuario + "<br>\r"
                    + "Data Fechamento do caixa: " + data1 + "<br>\r" 
                    + "Hora Fechamento do caixa: " + hora1 + "<br>\r"
                    + "-------------------------------- <br>"
                    + "        FECHAMENTO DE CAIXA        <br>\r"
                    + "--------------------------------<br>\r"
                    + "CAIXA      CARTÃO      DINHEIRO <br>\r"
                    + conteudoFor 
                    + "TOTAL DO CAIXA = " + ((bLMascaras.arredondamentoComPontoDuasCasas((float) modelCaixa.getCartao()))+ (bLMascaras.arredondamentoComPontoDuasCasas((float) modelCaixa.getDinheiro())))  + "<br>\r"
                    + "--------------------------------<br> \r"
                    + "</html>");
            //chamar metodo para enviar
            // /n/r para novas linhas e /f para fim da pagina  
        } catch (Exception e) {
            textoParaEnviar = "erro";
            JOptionPane.showMessageDialog(null, "Erro ao imprimir");
